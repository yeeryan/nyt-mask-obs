...
```{r}
nyc.covid19.mask_sample <- nyc.covid19.mask %>% group_by(zip) %>% sample_n(1)

mask_linmodel <- lm(COVID_CASE_RATE ~ obs_mask, data = nyc.covid19.mask_sample)
...
```{r}
ggplot(nyc.covid19.mask_sample, aes(x = obs_mask, y = COVID_CASE_RATE)) + 
  geom_point() +
  stat_smooth(method="lm", se = FALSE) +
  ggtitle("Observed Mask Rates & Case Rate") +
  xlab("Observed Mask Rate") +
  ylab("COVID Case Rate") +
  scale_x_continuous(labels = scales::percent)
...
```{r}
nyc.covid19.data_sample <- nyc.covid19.data %>% group_by(zip) %>% sample_n(1) %>%  drop_na()
...
```{r}
ggplot(data = nyc.covid19.data_sample)
```
