...
```{r logi model}
mask.rate_logmodel <- glm(avg_rate ~ obs_mask, family=binomial(link='logit'), data = nyc.avg.rate.mask)

summary(mask.rate_logmodel)

```
```{r chart, echo=FALSE, results='HIDE', warning=FALSE}
ggplotly(
ggplot(nyc.avg.rate.mask, aes(x = obs_mask, y = avg_rate)) + 
  geom_point() +
  stat_smooth(method="glm", method.args = list(family="binomial"), se=FALSE) +
  ggtitle("Observed Mask Rates & Avg Change in Case Rate") +
  xlab("Observed Mask Rate") +
  ylab("Average Change in Case Rate")
  )
...
#         color = ~area,
#         hoverinfo = 'text',
#     text = ~paste('</br> Area: ', area,
#                   '</br> Mask Rate: ', obs_mask*100,'%',
#                   '</br> Avg Rate: ', avg_rate*100,'%'),
#         visible = T) %>% 
#   layout(
#     title = 'Observed Mask Rates & Avg Case Rate by Area',
#     showlegend = TRUE,
#     yaxis = list(title = "Average Weekly Case Rate Change", ticksuffix ="%"),
#     xaxis = list(title = "Mask Observation Rate", ticksuffix ="%")
# )
...
```{r}
mask_linmodel <- lm(COVID_CASE_RATE ~ obs_mask, data = (nyc.covid19.mask %>% group_by(zip) %>% sample_n(1)))

summary(mask_linmodel)
...
```{r}
population_model <- lm(COVID_CASE_RATE ~ POP_DENOMINATOR, data = nyc.covid19.data)
