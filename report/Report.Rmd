...
nyc.covid19.mask_sample <- nyc.covid19.mask %>% group_by(zip) %>% sample_n(1) %>% drop_na() %>% ungroup() %>% filter(!zip == 11694)
...
```{r}
plot_ly(nyc.covid19.mask_sample,
       x = ~obs_mask,
       y = ~COVID_CASE_RATE,
       type = 'scatter',
       mode = 'markers',
  ...
  add_trace(data = nyc.covid19.mask_sample,
            x = ~obs_mask,
            y = fitted(mask_linmodel),
            type = 'scatter',
            mode = 'lines',
            name = "Regression Line",
            hovertemplate = ~paste('Mask Rate', obs_mask,
                                   '<br>Projected Case Rate', fitted(mask_linmodel))) %>% 
  layout(
    title = "Average Change Rate & Mask Rate with Regression Line",
    xaxis = list(title = "Observed Mask Rate", tickformat = '%'),
